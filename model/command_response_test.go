...
func TestCommandResponseFromJson(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			"",
			nil,
			sToP("error at offset 0: unexpected end of JSON input"),
		},
		{
			"malformed response",
			`{"text": }`,
			nil,
			sToP("error at offset 10: invalid character '}' looking for beginning of value"),
		},
		{
			"invalid response",
			`{"text": "test", "response_type": 5}`,
			nil,
			sToP("json: cannot unmarshal number into Go struct field CommandResponse.response_type of type string"),
		},
		{
			"ephemeral response",
			`{
